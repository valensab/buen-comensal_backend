...
def restaurants_list_10(request):
    ...
    if request.method == 'GET':
        ...
        for restaurant in restaurants_serializer.data:
            imagen = GalleryRestaurant.objects.filter(restaurant.get('id_restaurant'))
            print(imagen)
            list.append({'id_restaurant': restaurant.get('id_restaurant'), 'name': restaurant.get('name'), 'imagen': imagen })
