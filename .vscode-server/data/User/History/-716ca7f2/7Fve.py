...
def restaurants_list_10(request):
    ...
    if request.method == 'GET':
        ...
        for restaurant in restaurants_serializer.data:
            images = GalleryRestaurant.objects.filter(restaurant.get('id_restaurant'))
            imagen_serializer = ImagenListSerializer(images, many=True)
            print(imagen_serializer.data)
            list.append({'id_restaurant': restaurant.get('id_restaurant'), 'name': restaurant.get('name'), 'imagen': imagen_serializer.data})
