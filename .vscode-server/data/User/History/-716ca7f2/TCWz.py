...
from restaurants.api.serializers import RestaurantFieldSerializer, ImagenListSerializer, TagsListSerializer
...
def tags_user_delete(request, pk=None):
    ...
    if tags:
        if request.method == 'DELETE':
            tags.delete()
            return Response({'message':'La etiqueta fue eliminada correctamente!'},status = status.HTTP_200_OK)
    return Response({'message':'No se ha encontrado ninguna etiqueta asociada con estos datos'},status = status.HTTP_400_BAD_REQUEST)
