...
class UserManager(BaseUserManager):
    def _create_user(self, email,password, name,last_name, is_active, is_commensal, is_staff, is_superuser, **extra_fields):
        user = self.model(
            email = email,
            name = name, 
            last_name = last_name,
    ...
    def create_user(self, email,name,last_name,password=None, **extra_fields):
        return self._create_user(email,password, name,last_name,False,False,False, False **extra_fields)

    def create_superuser(self,email, name, last_name, password=None, **extra_fields):
        return self._create_user( email, password, name, last_name,True, True, True, True,**extra_fields)
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    phone_number = models.CharField("Número de teléfono", max_length=10, null = True)
