...
class RestaurantInfoAllAPIView(APIView):      
    def get(self, request):
        restaurant = Restaurant.objects.all()[:10]
        restaurant_serializer = RestaurantFieldSerializer(restaurant,many = True)
        user_restaurant = User.objects.filter(id = restaurant_serializer.data['user_id'] )
        return Response({
                        'id_restaurant': restaurant.id_restaurant,
                        'menu': restaurant.menu.url if restaurant.menu != '' else '',
                        # 'name_representative': restaurant.name_representative,
