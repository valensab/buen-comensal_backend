...
from restaurants.api.serializers import RestaurantSerializer,  RestaurantFieldSerializer, ImagenRestaurantSerializer, ImagenListSerializer, UpdateContactSerializer, UpdateRestaurantSerializer, UpdateMenuRestaurantSerializer, UpdateDescriptionRestaurantSerializer, TagsSerializer, SearchTagsSerializer, UpdateInfoRestaurantSerializer, TagsListSerializer
...
class TagsRestaurantAPIView(APIView):
    ...
    def post(self, request,*args,**kwargs):
        ...
        if serializer.is_valid():
            tags = TagsRestaurant.objects.filter(tags = request.data['tags'], restaurant= request.data['restaurant']).first()
            if tags:
                return Response({"message": "Está etiqueta ya existe"},status=status.HTTP_400_BAD_REQUEST)
            else: 
                serializer.save()
                return Response({"message": "Etiquetas agregadas con éxito"}, status=status.HTTP_201_CREATED)
